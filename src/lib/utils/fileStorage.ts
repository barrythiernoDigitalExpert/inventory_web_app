...
export const saveRoomImages = async (base64Images: string[], propertyRef: string, roomCode: string): Promise<string[]> => {
  try {
...
export const saveItemImage = async (base64Image: string, propertyRef: string, roomCode: string, itemId: string): Promise<string> => {
  try {
    const buffer = base64ToBuffer(base64Image);
...
// Supprime une image d'une pièce spécifique
export const deleteRoomImage = async (imagePath: string): Promise<boolean> => {
  try {
    const relativePath = imagePath.startsWith('/uploads/') 
      ? imagePath.substring('/uploads/'.length) 
